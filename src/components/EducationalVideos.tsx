...
const videoTutorials: VideoTutorial[] = [
  {
    ...
    level: "Beginner",
    category: "basics",
    videoUrl: "https://www.youtube.com/watch?v=TOx3pZlDHY4",
...
export const EducationalVideos: React.FC = () => {
  ...
  const filteredVideos =
    activeCategory === "all" ? videoTutorials : videoTutorials.filter((video) => video.category === activeCategory);
    ...
    return (
      <Card className="hover:shadow-lg transition-shadow cursor-pointer" onClick={() => setSelectedVideo(video)}>
        <div className="relative">
          ...
          <Badge className={`absolute top-2 right-2 ${levelColors[video.level]}`}>{video.level}</Badge>
        </div>
